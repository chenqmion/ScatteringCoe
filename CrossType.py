...
for w in w_list:
    #%% Numerical
    if (load == 'lamb2'):
        Zr = tk.Waveguide_Z(w,L/2, load='lamb4')/2
        M = tk.TM_Hori(1/(1j*w*C_1)) @ tk.TM_Vert(Zr) @ tk.TM_Hori(1/(1j*w*C_2))
    elif (load == 'lamb4'):
        Zr = tk.Waveguide_Z(w,L, load='lamb4')
        M = tk.TM_Hori(1/(1j*w*C_1)) @ tk.TM_Vert(Zr) @ tk.TM_Hori(1/(1j*w*C_2))
    else:
    ...
    params = tk.Ana_Params_Cross(L,C_1,C_2, load=load)
    # params[1] = params[1] + 1E6*(np.pi)
    Ana_S.append(tk.Ana_SC_Cross(w, params))

Num_S = np.array(Num_S)
Ana_S = np.array(Ana_S)

# np.save('Num_S_cross.npy', Num_S)
# np.save('Ana_S_cross.npy', Ana_S)
...
plt.savefig('Fig_cross.pdf', bbox_inches='tight')
plt.show()
