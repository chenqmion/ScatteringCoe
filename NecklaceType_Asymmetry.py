...
C_2 = 1E-14
load = 'lamb2'
w_list = np.linspace(6.637,6.683,201) * 1E9 * (2*np.pi)
...
for w in w_list:
    #%% Numerical
    if (load == 'lamb2'):
        Mr = tk.TM_Hori(1/(1j*w*C_1)) @ tk.TM_Waveguide(w,L) @ tk.TM_Hori(1/(1j*w*C_2))
    elif (load == 'lamb4'):
        Mr = tk.TM_Hori(1/(1j*w*C_1)) @ tk.TM_Waveguide(w,L) @ np.array([[1,0],[1j*w*C_2,1]])
    else:
        print('geometry error')
        
    M_1 = tk.TM_Waveguide(w,l_1) @ tk.TM_Hori(1j*w*L_1)
    M_2 = tk.TM_Hori(R_2) @ tk.TM_Waveguide(w,l_2)
    M = M_1 @ Mr @ M_2
...
# np.save('necklace_lamb2_asymmetry.npy', [Num_S[:,0],w_list/2/np.pi/1E9])
